...
WORKDIR /src/yoki-event-worker

# Build with static binaries attached (to be able to use small images)
RUN CGO_ENABLED=0 GOOS=linux go build -a -installsuffix cgo -o yoki-event-worker .


FROM alpine

COPY --from=builder /src/yoki-event-worker/yoki-event-worker /app/
